...
interface LeagueEntry {
  code: string;
  currentRate: number;
  change: number; // % change over period
}
...
export default function CurrencyLeagueTable({
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const to = todayDate();
      const codes = ALL_CURRENCY_CODES.filter((c) => c !== base);

      const results = await Promise.allSettled(
        codes.map(async (quote) => {
          const [rateResult, seriesResult] = await Promise.all([
            fetchLatestRate(base, quote),
            fetchTimeSeries(base, quote, from, to, group),
          ]);
